...
sealed unsafe class CosmicMissionNameMutation
{
    private readonly CosmicMissionNameAddonContract _addonContract;
    private NameLayoutSnapshot? _nameLayoutSnapshot;

    public CosmicMissionNameMutation(CosmicMissionNameAddonContract addonContract)
    {
    ...
    public bool Apply(
        AtkUnitBase* addon,
        string translation,
        CosmicMissionNameAppearance appearance)
    {
        ...
        translationNode->AlignmentType = nativeNameNode->AlignmentType;
        SetColouredText(translationNode, $"～ {translation} ～", appearance.ColourKey);
        translationNode->ResizeNodeForCurrentText();
        translationNode->SetWidth(nativeNameNode->AtkResNode.Width);
