using BilingualTooltips.Modules.Dialogue;
using Dalamud;
using Dalamud.Interface.ManagedFontAtlas;
using Dalamud.Interface.Style;

namespace BilingualTooltips.Assets;

public static class Data
{
    public static readonly IFontHandle DialogueCjkBody20 = CreateDialogueCjkBodyFont();

    private static IFontHandle CreateDialogueCjkBodyFont() =>
        Service.PluginInterface.UiBuilder.FontAtlas.NewDelegateFontHandle(toolkit =>
        {
            toolkit.OnPreBuild(preBuild =>
            {
                var font = preBuild.AddDalamudAssetFont(
                    DalamudAsset.NotoSansCjkRegular,
                    new SafeFontConfig
                    {
                        SizePx = 20.0f,
                        GlyphRanges = DialogueFontGlyphRanges.Current,
                    });

                preBuild.AddGameSymbol(new SafeFontConfig
                {
                    SizePx = 20.0f,
                    MergeFont = font,
                });
                preBuild.Font = font;
            });
        });

    public static readonly StyleModel DefaultTheme =
        StyleModel.Deserialize(
            "")
        !;
}
